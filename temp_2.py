...
# df = pd.read_csv("./test_result_csv/td1_roughness_enforced_5_normalised_gin.csv")
...
# ute_df = pd.read_csv("./ids/only_ute_1.25mm.csv")
...
# csv_paths = [f"./log_local_complexity/roughness_enforced_latest_model_UTE_marissa/{'-'.join(scan_path.split('/')[1:])[:-4]}.csv" for scan_path in ute_filepaths]
...
# fig.tight_layout()
# fig.savefig("./log_local_complexity/all_lc_vs_iou_for_all_scans.png", dpi=150)
# plt.close(fig)
# print("Saved combined scatter plot")

# Histogram of LC values across ALL csv files in the folder
import glob
all_csv_paths = sorted(glob.glob("./log_local_complexity/roughness_enforced_latest_model_COPD_CT_100/*.csv"))
...
fig_hist.savefig("./log_local_complexity/all_lc_histograms_COPD_CT_100.png", dpi=150)
plt.close(fig_hist)
...
# ute_csv_paths = sorted(glob.glob("./log_local_complexity/roughness_enforced_latest_model_UTE_marissa/*.csv"))
# ct_csv_paths = sorted(glob.glob("./log_local_complexity/roughness_enforced_latest_model_COPD_CT_100/*.csv"))
...
# fig_overlap.savefig("./log_local_complexity/ute_vs_ct_lc_histogram_overlap.png", dpi=150, bbox_inches='tight')
